<template>
  ...
  >
    <div class="w-full mb-1">
      <div class="mb-4">
        <Breadcrumb name="Quản lý rạp chiếu" />
        <h1
  ...
  <div v-if="cinemasList.length > 0 && !isLoading" class="flex flex-col">
  ...
  <EmptyList v-if="cinemasList.length <= 0 && !isLoading" />
</template>
<script>
...
export default {
  ...
  methods: {
    async loadData() {
      this.isLoading = true;
      await getAllCinemas().then(async (res) => {
        ...
        this.cinemasList = res.data.slice(start, end);
    ...
    async deleteCinemas(id) {
      await deleteCinemas(id)
        .then(async (res) => {
          ...
          await this.loadData();
          if (this.cinemasList.length === 0 && this.currentPage > 1) {
