<template>
    ...
    <div>
        ...
        <div>
           ...
           <ButtonHandleCreate @handleCreate="createNewCinemas" :genresData="genres" :class="'mt-4 mb-4'" :formFields="formFields"/>
        </div>
        <div>
            ...
            <div v-for="(item, index) in filmList" :key="index" class="flex justify-around items-center gap-2 border-t border-t-[#0000002f]  px-[16px] hover:bg-[#e5e5e5] py-[8px]">
                ...
                <span class="w-[25%]">
                    <ButtonHandleModal @handleDelete="deleteCinemas" @handleUpdate="updateCinemas" :data="item" :formFields="formFields" :genresData="genres"/>
                </span>
            </div>
        </div>
...
export default {
    ...
    methods: {
        loadData(){
            axios.get("https://localhost:7253/api/Cinemas")
        ...
        createNewCinemas(form_data){
            ...
            axios.post("https://localhost:7253/api/Cinemas",formData , {
        ...
        updateCinemas(id, form_data){
            ...
            formData.append("address", form_data.address);
            formData.append("formFile", form_data.image);
            axios.put(`https://localhost:7253/api/Cinemas/${id}`,formData , {
            headers: {'Content-Type': 'multipart/form-data',}
            })
            .then((res) =>  {
                this.toggleModalMessage = true;
        ...
        deleteCinemas(id){
            axios.delete(`https://localhost:7253/api/Cinemas/${id}`)
            .then((res) =>  {
                this.toggleModalMessage = true;
                this.message = res.data;
                this.loadData();
