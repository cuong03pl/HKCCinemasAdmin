<template>
    ...
    <div>
        ...
        <div>
           ...
           <ButtonHandleCreate @handleCreate="createNewCategory"  :class="'mt-4 mb-4'" :formFields="formFields"/>
        </div>
        <div>
            ...
            <div v-for="(item, index) in categoryList" :key="index" class="flex justify-between items-center gap-2 border-t border-t-[#0000002f]  px-[16px] hover:bg-[#e5e5e5] py-[8px]">
                
                <span class="w-[30%]">{{ item.name }}</span>
                <span class="w-[45%]">{{ }}</span>
                <span class="w-[25%]">
                    <ButtonHandleModal @handleDelete="deleteCategory" @handleUpdate="updateCategory" :data="item" :formFields="formFields"/>
                </span>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        createNewCategory(form_data){
            ...
            axios.post("https://localhost:7253/api/Categories",formData , {
        ...
        updateCategory(id, form_data){
            var formData = new FormData();
            formData.append("name", form_data.name);
            axios.put(`https://localhost:7253/api/Categories/${id}`,formData , {
            headers: {'Content-Type': 'multipart/form-data',}
            })
            .then((res) =>  {
                this.toggleModalMessage = true;
        ...
        deleteCategory(id){
            axios.delete(`https://localhost:7253/api/Categories/${id}`)
            .then((res) =>  {
                this.toggleModalMessage = true;
                this.message = res.data;
                this.loadData();
